...
from draw_detection_with_gt import draw_detections
...
def _get_image_blob(im):
    ...
    stride = cfg.TRAIN.IMAGE_STRIDE #add
    processed_ims = []
    im_scale_factors = []
...
def im_detect(net, im, boxes=None,num_classes=21):
    ...
    #print("blobs['data'].shape is ",blobs['data'][0,0,0,0])
    #print("blobs['data'].shape is ",blobs['data'][0,0,0,1])
    #print("blobs['data'].shape is ",blobs['data'][0,0,0,100])
...
# / im_scales[0]
    ...
    if cfg.TEST.BBOX_REG:
        ...
        if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED: #add
            means = np.tile(
      ...
      #  print boxes.shape,box_deltas.shape
        boxes.resize([boxes.shape[0],boxes.shape[1]])
        pred_boxes = bbox_transform_inv(boxes, box_deltas)
        s = (blobs['data'].astype(np.float32, copy=False).shape[2],blobs['data'].astype(np.float32, copy=False).shape[3],blobs['data'].astype(np.float32, copy=False).shape[1]) #add
...
def test_net(net, imdb,net_name, max_per_image=100, thresh=0.5, vis=False):
    ...
    for i in xrange(num_images):
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            cls_dets = bbox_vote(dets_NMSed, cls_dets) #add
           # print cls_scores
            if vis:
    ...
    thresholds = [0.1 , 0.2 ,0.5, 0.7, 0.9]
    ans = imdb.evaluate_mAP(all_boxes = all_boxes, thresholds = thresholds)
    print 'mAP is {}, \n{}, according to {}'.format(ans['mAP'], ans['area_class'], thresholds)

    print "writing to file"
    res_file = imdb._write_detect_results_file(all_boxes = all_boxes, conf_thresh = 0.1, iou_thresh = 0.1, net_name = net_name)
    draw_pic_save_path = "/home/ayogg/USB/USB2018_Dataset/test_jpg_FPN"
    draw_detections(res_file, draw_pic_save_path)
